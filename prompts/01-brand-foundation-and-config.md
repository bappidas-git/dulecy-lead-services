# Prompt 01 — Brand Foundation & Configuration (Nilachal → Dulecy)

## 1. Objective

Re-point the repository's **identity layer** from Nilachal Infracon to Dulecy Lead
...
already belongs to Dulecy, and all later prompts can build on the tokens.

## 2. Background

- The repo is a CRA (react-scripts 5) React 18 app, previously the Nilachal
  Infracon one-pager. The final Dulecy site is defined by the static mockup in
...
- Dulecy canonical facts (from the mockup — copy exactly):
  ...
  - Email: **dulceyleadservices@gmail.com** (intentional "dulcey" spelling — do not correct)
  - Site URL: **https://www.dulecy.com**
  - Logos (Cloudinary): color `.../v1785484838/Dulecy-Logo_qr2ka7.png`, white
    `.../v1785484839/Dulecy-Logo-White_uxpsb6.png`, icon
    `.../v1785484838/Dulecy-Logo-Icon_hylrpw.png` (base
...
- `legalName` & `brandName`: `'Dulecy Lead Services'`; remove `flagshipBrand`
  only if no file imports it (otherwise set it to `'Dulecy Lead Services'` and
...
- `siteUrl: 'https://www.dulecy.com'`; `logo`/`logoWhite` → the Dulecy
  Cloudinary URLs above; add `logoIcon` for the icon mark.
- Remove `cin` and `mapsQuery`; set `address` fields to empty strings (helper
  `fullAddress` already skips empties). Set `whatsapp` to the Dulecy phone and
...
- Generate a **new** 48-char random `REACT_APP_LEADS_ADMIN_KEY` and set it in
  ...
  "Dulecy Lead Services" instead of Nilachal. Do not change any other logic in
  `leads.php`.

### 4.3 `package.json` & `public/manifest.json`

- `name: "dulecy-lead-services-website"`, description "Dulecy Lead Services —
  multi-page business-consulting website (React 18 + MUI + GSAP) with lead
  management admin panel.", keywords (dulecy, lead-services,
...
- Manifest: `short_name: "Dulecy"`, `name: "Dulecy Lead Services"`,
...
- Add the Dulecy set exactly: `--ink:#0B0B0C; --grey-1:#2A2A2E;
...
- Re-point the legacy tokens to Dulecy values: `--color-primary`/`-dark`/
  ...
  corresponding Dulecy value so nothing renders navy/green anymore.
...
`navy`, …) as aliases mapped to Dulecy values because `sx` props reference
...
- Splash loader: swap the logo `src` to the Dulecy **icon** logo, keep the
...
- `npm start`: home page renders (still Nilachal layout) with Dulecy logo in
...
App builds and runs with Dulecy identity (facts, tokens, fonts, logos,
...
Work on branch `dulecy/01-brand-foundation`, commit with clear messages, push,
